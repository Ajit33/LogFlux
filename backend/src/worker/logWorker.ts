  ...
  async start() {
    ...
    this.worker = new Worker<Log>(
      ...
      async (job: Job<Log>) => {
        ...
        if (this.pendingBatch.length >= config.queue.batchSize) {
          logger.info(
            `Batch size ${config.queue.batchSize} reached — flushing`,
          );
    ...
    this.flushTimer = setInterval(async () => {
      if (this.pendingBatch.length > 0) {
        logger.info(
          `Flush interval reached — flushing ${this.pendingBatch.length} log(s)`,
        );
        await this.flushToElasticsearch();
      } else {
    ...
    logger.info(
      `Worker running — batchSize: ${config.queue.batchSize}, flushInterval: ${config.queue.flushInterval}ms`,
    );
  }
  ...
  private async flushToElasticsearch() {
    ...
    try {
      ...
      if (response.errors) {
        ...
        logger.error(
          `Bulk index: ${failed.length} failures out of ${batchToIndex.length}`,
        );
      } else {
      ...
      this.pendingBatch.unshift(...batchToIndex);
      logger.warn(
        `Returned ${batchToIndex.length} logs to pending batch for retry`,
      );
    }
  }
